...
/**
 * Highscore-Einträge werden als Liste zusammengefasst und in einer Datei
 * gespeichert. Datei kann geschrieben, gelesen, geprüft werden.
 * 
 * @author DSIW
 * 
 */
public class Highscore
{
  ...
  /**
   * Die Highscore-Datei wird angelegt, wenn diese nicht vorhanden ist und diese
   * wird gelesen.
   */
  public Highscore()
  {
  ...
  /**
   * Die Highscore-Datei wird angelegt, wenn diese nicht vorhanden ist und ein
  ...
  private void initFile ()
  {
    ...
    if (file.exists() && file.canRead() && file.canWrite()) {} else {
  ...
  public void add (Gamer gamer, int level, int points)
  {
    ...
    if (isNotEntryInList(he)) {
      if (list.size() == Data.HIGHSCORE_MAX_GAMER) {
        ...
        // Teste, ob egi Punktzahl größer ist, als die geringste Pktzahl in
        // Highscore
  ...
  {
    HighscoreEntry tempH = new HighscoreEntry(new Gamer("temp"),
  ...
  public static boolean testFile (File file)
  {
    ...
    if (!file.isFile()) {
      ...
      JOptionPane.showMessageDialog(null,
          "Die Pfadangabe ist keine Datei. Bitte wählen Sie eine Datei aus!",
    ...
    if (file.exists()) {
      if (!file.canRead()) {
        ...
        JOptionPane
            .showMessageDialog(
                null,
                "Die Datei konnte nicht gelesen werden. Bitte wählen Sie eine andere Datei aus!",
                "Keine Leseberechtigung!", JOptionPane.ERROR_MESSAGE);
      }
    } else {
      try {
        file.createNewFile();
        ...
        JOptionPane
            .showMessageDialog(
                null,
                "Die Datei konnte nicht geschrieben werden. Bitte wählen Sie eine andere Datei aus!",
                "Keine Schreibberechtigung!", JOptionPane.ERROR_MESSAGE);
      }
      finally {
        fileIsOK = false;
      }
  ...
  public void read ()
  {
    try {
      ...
      while (scanner.hasNextLine()) {
        ...
        list.add(new HighscoreEntry(new Gamer(s[0]), Integer.parseInt(s[1]),
      ...
      if (!file.canRead()) {
        ...
        JOptionPane
            .showMessageDialog(
                null,
                "Die Datei konnte nicht gelesen werden. Bitte wählen Sie einen anderen Pfad aus!\n"
                    + "Einen neuen Pfad können Sie in den Optionen festlegen.",
                "Keine Leseberechtigung!", JOptionPane.ERROR_MESSAGE);
      }
      if (fileIsOK) write();
    }
