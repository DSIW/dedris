...
/**
 * Klasse, die die gefallenen Steine speichert und verändert. Die gefallenen
 * Steine werden in einem Array gespeichert.
 * 
 * @author DSIW
 * 
 */
public class SavedFigure
{
   ...
   * 
   * @param rowJustDeleted
   *          true, wenn gerade geloescht wurde.
   */
   ...
   *           Diese Exception wird geworfen, wenn kein relativer Stein (1-3)
   *           existiert.
   */
  public void save (Figure activeFigure) throws NoRelativeStoneException
  {
    try {
      // Stein 0 speichern
      bottomStones[(int) (activeFigure.getStone(0).getV().getY() / Stone.WIDTH)][(int) (activeFigure
          .getStone(0).getV().getX() / Stone.WIDTH)] = activeFigure.getStone(0);
      ...
      for (int i = 1; i <= 3; i++) {
        bottomStones[(int) (activeFigure.getStone(0).getV().getY()
            / Stone.WIDTH + activeFigure.getRelStonePos(i).getY())][(int) (activeFigure
            .getStone(0).getV().getX()
            / Stone.WIDTH + activeFigure.getRelStonePos(i).getX())] = activeFigure
            .getStone(i);
        // Steine 1,2,3 oben angekommen?
        if ((int) (activeFigure.getStone(0).getV().getY() / Stone.WIDTH + activeFigure
            .getRelStonePos(i).getY()) <= 0) {
          Main.stop();
        }
  ...
  /**
   * Prüft, ob die Reihe row voll ist. Also alle Positionen mit Steinen gefüllt
   * sind.
   * 
   * @param row
   *          Die Reihe, die überprüft werden soll.
      ...
      for (int c = 0; c < COLUMNS; c++) {
        ...
        // if(Main.DEBUG && c == 0) System.out.print("1 | ");
          ...
          if (bottomStones[r - 1][c] == null) {
            ...
            bottomStones[r][c].setV(new Vertex(
                bottomStones[r][c].getV().getX(), bottomStones[r][c].getV()
                    .getY() + Stone.WIDTH));
          }
        }
        // if(Main.DEBUG && c == 0 && r > 0)
        // System.out.print((r-1<10?"0"+(r-1):r-1)+" > "+(r<10?"0"+r:r)+" || ");
        // if(Main.DEBUG && c == 0 && r <= 0)
        // System.out.print("nl > "+(r<10?"0"+r:r)+" || ");
        // if(Main.DEBUG)
        // System.out.print(/*"NS["+(r<10?"0"+r:r)+"]["+(c<10?"0"+c:c)+"] = "+*/(bottomStones[r][c]
        // != null?"S":" ")+" | ");
      }
      // Main.debugPrintln();
    }
  ...
  /**
   * Prüft alle Reihen, ob diese voll sind. Wenn eine Reihe voll ist, wird diese
   * gelöscht und alle Steine darüber werden um eine Steinbreite nach unten
   * versetzt.
   */
  ...
  /**
   * Prüft, ob die aktuelle Figur einen der gespeicherten Steine im nächsten
   * Schritt berührt.
   * 
   * @param activeFigure
   *          Aktive Figur
  ...
  public boolean checkBottom (Figure activeFigure)
  {
    Vertex startV = new Vertex(
        (int) (activeFigure.getStone(0).getV().getX() / Stone.WIDTH),
        (int) (activeFigure.getStone(0).getV().getY() / Stone.WIDTH));

    // Check Orientierungsstein auf mögliche Berührung im nächsten Schritt
    // (Fallen)
    ...
    for (int c = minX; c <= maxX; c++) {
      ...
      // Check rel. Steine
      if (maxY >= 0 && maxY < ROWS) {
        if (bottomStones[maxY][startV.getX() + c] != null) { // Prüfe auf
                                                             // mögliche
                                                             // Berührung
          return true;
        }
  ...
  public boolean checkLeft (Figure activeFigure)
  {
    ...
    for (int r = minY; r <= maxY; r++) {
      ...
      if (minXabs >= 0 && minXabs < COLUMNS) {
        try {
          if (bottomStones[startV.getY() + r][minXabs] != null) { // Prüfe auf
                                                                  // mögliche
                                                                  // Berührung
  ...
  public boolean checkRight (Figure activeFigure)
  {
    ...
    for (int r = minY; r <= maxY; r++) {
      ...
      if (maxXabs >= 0 && maxXabs < COLUMNS - 1) {
        try {
          if (bottomStones[startV.getY() + r][maxXabs] != null) { // Prüfe auf
                                                                  // mögliche
                                                                  // Berührung
      ...
      } else if (maxXabs > COLUMNS - 1) { // Prüfe, ob Stein rechts angekommen
                                          // ist
