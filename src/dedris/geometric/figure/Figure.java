...
import dedris.game.RandomInteger;
 ...
 * an dem sich alle anderen drei Steine ausrichten. Die Figur enthält einen
 * Drehstein, der den Drehpunkt darstellt. Jeder Stein hat eine maximale
 * Drehanzahl. Die aktuelle Drehposition wird gespeichert.
 * 
 * @author DSIW
 */
public abstract class Figure
{
  ...
  void calcDimension ()
  {
    setWidth((Math.abs(getMaxRelStonePos().getX())
        + Math.abs(getMinRelStonePos().getX()) + 1)
        * Stone.WIDTH);
    setHeight((Math.abs(getMaxRelStonePos().getY())
        + Math.abs(getMinRelStonePos().getY()) + 1)
        * Stone.WIDTH);
  }
  ...
  Stone getRelativeStone (final int x, final int y, Color c, int stoneIndex)
  {
    ...
    return new Stone(this.s0.getV().getX() + x * Stone.WIDTH, this.s0.getV()
        .getY() + y * Stone.WIDTH, s0.isFilled(), c, stoneIndex);
  }
   ...
   * 
   * @param refresh
   *          Performante Option, damit bei mehrfacher Rotation die Steine nicht
   *          aktualisiert werden.
   */
  ...
  /**
   * Bekomme max. X-Wert des entferntesten rel. Steins und max. Y-Wert des
   * entferntesten rel. Steins als Vertex
   * 
   * @return Max. X-Wert
   */
  ...
  /**
   * Bekomme Min. X-Wert des entferntesten rel. Steins und min. Y-Wert des
   * entferntesten rel. Steins als Vertex
   * 
   * @return Min. X-Wert
   */
   ...
   * 
   * @param relStonePosRow
   *          relative Stein-Position in Zeile
   * @return max. X-Wert
   */
   ...
   */
  // public boolean checkBottom(GeometricObject window, java.util.List<Figure>
  // savedFigures) {
  ...
  // }
  //
  // // Vergleich mit allen Steinen der aktiven Figur mit allen Steinen der
  // gespeicherten Figur(en).
  ...
  public boolean isOutOfFieldHorizontalLeft (GeometricObject window)
  {
    ...
    for (int i = 0; i <= 3; i++) {
      // Main.debugPrintln(this.getStone(i).getV().getX() + " <= " +
      // window.getV().getX());
  ...
  /**
   * Prüft, ob sich Figur links ausserhalb des Spielfeldes befinden würde, wenn
   * sie sich einen Schritt weiterbewegt.
   * 
   * @param window
   *          Spielfeld
   * @return true, wenn ausserhalb
  ...
  public boolean isOutOfFieldHorizontalLeftBefore (GeometricObject window)
  {
    ...
    for (int i = 0; i <= 3; i++) {
      if (this.getStone(i).getV().getX() - this.getStone(i).getWidth() <= window
          .getV().getX()) {
        return true;
      }
    }
  ...
  /**
   * Prüft, ob sich Figur rechts ausserhalb des Spielfeldes befinden würde, wenn
   * sie sich einen Schritt weiterbewegt.
   * 
   * @param window
   *          Spielfeld
  ...
  {
    for (int i = 0; i <= 3; i++) {
      if (this.getStone(i).getV().getX() + this.getStone(i).getWidth()
          + this.getStone(i).getWidth() >= window.getV().getX()
          + window.getWidth()) {
        return true;
      }
    }
  ...
  public boolean isOutOfFieldHorizontal (GeometricObject window)
  {
    if (isOutOfFieldHorizontalLeft(window)
        || isOutOfFieldHorizontalRight(window)) {
      return true;
    }
    return false;
  }
  ...
  {
    for (int i = 0; i <= 3; i++) {
      if (this.getStone(i).getV().getY() + this.getStone(i).getHeight() >= window
          .getV().getY() + window.getHeight()) {
        return true;
      }
    }
    return false;
  ...
  /**
   * Alle anderen Steine orientieren sich an dem Orientierungsstein und richten
   * sich neu aus.
   */
  ...
  {
    return new Stone(
        (int) this.s0.getV().getX() + (int) v.getX() * Stone.WIDTH,
        (int) this.s0.getV().getY() + (int) v.getY() * Stone.WIDTH,
        s0.isFilled(), c, stoneIndex);
  }
  ...
  /**
   * Bekomme die Position des relativen Steins, in Abhängigkeit von der
   * Rotation.
   * 
   * @param stone
   *          Index des relativen Steins
   * @return Vertex der Position
   */
  public abstract Vertex getRelStonePos (int stone)
  ...
  protected void rotateFinish ()
  {
    ...
    // Masse tauschen
    swapDimension();
    // Muss an der Stelle stehen, damit die Steine wieder neu ausgerichtet
    // werden
    refreshStones();
  }
