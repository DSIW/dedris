...
/**
 * Options-Fenster, in dem man verschiedene Einstellungen zum Spiel treffen
 * kann.
 * 
 * @author DSIW
 * 
...
public class Option extends MyNoGameJFrame
{
  ...
  JComponent setPath (Game g, final JTextField path)
  {
    ...
    JButton chooserB = new JButton("Auswählen");
    ...
    p.add(chooserB, BorderLayout.EAST);
    return p;
  }


  private void choosePath (JTextField path)
  {
    final JFileChooser fc = new JFileChooser(path.getText());
    fc.setFileSelectionMode(JFileChooser.FILES_ONLY);
    FileFilter filter = new FileNameExtensionFilter("Highscore file", "scores",
        "score", "txt");
  ...
  JComponent frameOperation (final MyJFrame f, final Game g,
      final JTextField eingabe, final JComboBox level,
      final JCheckBox randomColor, final JTextField path)
  {
    ...
    auswVerfButtonsFlow.add(new JLabel(" "), BorderLayout.CENTER); // Abstand
                                                                   // einfügen
   ...
   *          Grafik-Objekt
   * @param setPos
   *          Wahrheitswert, der angibt, ob das Fenster am Vaterfenster
  ...
  public Option(Game g, boolean setPos)
  {
    ...
    // Durch das BorderLayout konnte ich nicht alle Komponenten in das Layout
    // implementieren,
    ...
    last.add(frameOperation(this, g, eingabe, level, randomColor, path),
        BorderLayout.SOUTH);
