...
import dsiw.exception.NoTerminationException;
...
//	private Font font = new Font(new MyFont().getFont().getFontName(), Font.PLAIN, 14);
//	private Font font = new Font("Ubuntu", Font.PLAIN, 15);
	
	/**
	 * Setze volle Reihen
	 * @param fullRows volle Reihen
	 */
	...
	public void pause(Graphics g) {
		...
		Vertex point = new Vertex((Data.WINDOW_WIDTH-Data.WINDOW_WIDTH_PAUSE)/2, (Data.WINDOW_HEIGHT-Data.WINDOW_HEIGHT_PAUSE)/2);
		Rectangle rect = new Rectangle(point.getX(), point.getY(), Data.WINDOW_WIDTH_PAUSE, Data.WINDOW_HEIGHT_PAUSE);
	...
	/**
	 * Initialisierung der Statistik. Setze Farbe auf 'weiß'. Setze X-Anfangs-Abstand, Linienhöhen.
	 * Initialisiert Statistik-Array und berechne maximale Beschreibungslänge.
	 * @param g Grafik-Objekt
	 */
...
//		if(Game.DEBUG) System.out.println("maxL: "+maxLength);
	 ...
	 * @param g Grafik-Objekt
	 */
	public void grid(Graphics g) { this.grid(g, 1); }
	/**
	 * Zeichnet Raster mit vertikalen und horizontalen Linien.
	 * @param g Grafik-Objekt
	 * @param intervall Intervall, das bestimmt, wie oft die Linien gezeichnet werden sollen.
	 */
	public void grid(Graphics g, int intervall) { this.grid(g, intervall, true, true); }
	/**
	 * Zeichnet Raster als Hintergrund.
	 * @param g Grafik-Objekt
	 * @param intervall Intervall, das bestimmt, wie oft die Linien gezeichnet werden sollen.
	 * @param vertical Wenn true, dann werden die vertikalen Linien gezeichnet.
	 * @param horizontal Wenn true, dann werden die horizontalen Linien gezeichnet.
	 */
	public void grid(Graphics g, int intervall, boolean vertical, boolean horizontal) {
		if(intervall <= 0) throw new NoTerminationException();
		g.setColor(Color.DARK_GRAY);
		if(vertical) {
			for(int i = 1; i <= Data.ROWS; i += intervall) {
				g.drawLine((int)Data.FIELD.getV().getX(),
						(int)Data.FIELD.getV().getY() + i*Stone.WIDTH,
						(int)Data.FIELD.getV().getX() + Data.FIELD.getWidth(),
						(int)Data.FIELD.getV().getY() + i*Stone.WIDTH);
			}
		}
		if(horizontal) {
			for(int i = 1; i <= Data.COLUMNS; i += intervall) {
				g.drawLine((int)Data.FIELD.getV().getX() + i*Stone.WIDTH,
						(int)Data.FIELD.getV().getY(),
						(int)Data.FIELD.getV().getX() + i*Stone.WIDTH,
	...
	/**
	 * Zeichnet Spielfeld-Rahmen. Setze Rahmen um 1 Pixel nach links und oben, damit man den linken und oberen Rahmen nicht sieht.
	 * @param g Grafik-Objekt
	 */
	public void gameField(Graphics g) {
		g.setColor(Color.WHITE);
		g.drawRect(-1, -1, Data.WINDOW_WIDTH_FIELD+2, Data.WINDOW_HEIGHT_FIELD+2);
	}
	...
	public void gameOver(Graphics g) {
		...
		g.drawString(name, Data.WINDOW_WIDTH-getPixelWidthOfString(name, g)-5, Data.WINDOW_HEIGHT-getPixelLineHeight(g)+5);
	}
	 ...
	 * @param g Grafik-Objekt
	 * @param figure Figur, bei der gezeichnet werden soll.
	 */
