...
public class GeometricObject implements GameObject {
	 ...
	 * Standard-Eigenschaften sind, dass es keine Geschwindigkeit hat, 
	 * sowohl vertikal, als auch horizontal; keine Füllung; Farbe 'weiß'; Höhe und Breite auf 1 Pixel; Startpunkt liegt bei (0,0).
	 */
	 ...
	 * Erzeugt ein neues Rechteck mit verschiedenen Eigenschaften.
	 * Standard-Eigenschaften sind, dass es keine Geschwindigkeit hat, 
	 * sowohl vertikal, als auch horizontal; keine Füllung; Farbe 'weiß'; Höhe und Breite auf 1 Pixel.
	 * @param x Start-Punkt (horizontal)
	 * @param y Start-Punkt (vertikal)
	 */
	 ...
	 * Erzeugt ein neues Rechteck mit verschiedenen Eigenschaften. 
	 * Standard-Eigenschaften sind, dass es keine Geschwindigkeit hat, 
	 * sowohl vertikal, als auch horizontal; keine Füllung und die Farbe 'weiß'.
	 * @param x Start-Punkt (horizontal)
	 * @param y Start-Punkt (vertikal)
	 * @param width Breite
	 * @param height Höhe
	 ...
	 * @param width Breite
	 * @param height Höhe
	 * @param dx Geschwindigkeit (horizontal)
	 * @param dy (vertikal)
	 * @param isFilled Füllen mit Farbe
	 * @param c Farbe
	 */
	public GeometricObject(int x, int y, int width, int height,
			int dx, int dy, boolean isFilled, Color c) {
		setV(new Vertex(x, y));
	...
	public String toString() {
		return "Das Rechteck mit den Anfangspunkten [" + v.getX() + " | " + v.getY()
				+ "] hat eine Breite von " + width + " und eine Hoehe von "
				+ height + ".";
	}
	...
	boolean hasWithin(GeometricObject that) {
		...
		if (this.hasWithin(thatvlo) || this.hasWithin(thatvlu)
				|| this.hasWithin(thatvro) || this.hasWithin(thatvru)
				|| touchOhneEcke) {
			return true;
		} else {
			return false;
